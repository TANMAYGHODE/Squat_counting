...
STREAMMUX_WIDTH = 2160
STREAMMUX_HEIGHT = 1920
GPU_ID = 0
...
def calculate_angle(keypoints):
    ...
    a=[keypoints[11],keypoints[12],keypoints[6],keypoints[4]]
    b=[keypoints[13],keypoints[14],keypoints[12],keypoints[6]]
    c=[keypoints[15],keypoints[16],keypoints[14],keypoints[12]]
    
    for i in range(4):

        ba1 = np.array(a[i]) - np.array(b[i])
        bc1 = np.array(c[i]) - np.array(b[i])
    
        cosine_angle1 = np.dot(ba1, bc1) / (np.linalg.norm(ba1) * np.linalg.norm(bc1))
        angle1 = np.arccos(np.clip(cosine_angle1, -1.0, 1.0))
        angle.append(np.degrees(angle1))

    return angle

def calculate_squat_angles(keypoints):
    # Extract keypoint coordinates
    x7, y7 = keypoints[6]   # Shoulder (Right)
    x13, y13 = keypoints[12] # Hip (Right)
    x15, y15 = keypoints[14] # Knee (Right)

    # Compute angles
    angle1 = (180 / np.pi) * np.arctan(abs((y13 - y7) / (x7 - x13)))
    angle2 = (180 / np.pi) * np.arctan(abs((y15 - y13) / (x15 - x13)))
...
def set_custom_bbox(obj_meta):
    ...
    obj_meta.text_params.display_text = "Person: "+str(obj_meta.object_id)+" Number_of_squats:"+str(squatCount[obj_meta.object_id])
...
def parse_pose_from_meta(frame_meta, obj_meta):
    ...
    for i in range(num_joints):
        ...
        # Set text parameters
        display_meta.num_labels += 1  # Increase text count
        text_params = display_meta.text_params[display_meta.num_labels - 1]
        text_params.display_text = str(i)  # Text inside the circle
        text_params.x_offset = int(xc - 3)  # Center text in the circle
        text_params.y_offset = int(yc + 3)
        text_params.font_params.font_size = 20 # Adjust font size
        text_params.font_params.font_name = "Arial"
        text_params.font_params.font_color.set(1.0, 1.0, 1.0, 1.0)  # White color
        text_params.set_bg_clr = 1  # Enable background color
        text_params.text_bg_clr.set(0.0, 0.0, 0.0, 0.5)  # Black semi-transparent BG
        count=count+1
    ...
    try:
        ...
        py_nvosd_text_params.display_text = "Knee1={} Knee2={} Hips={} Stature={}".format(int(angle[0]), int(angle[1]), int(angle[2]),int(angle[3]))
        py_nvosd_text_params.x_offset = 10+x_offset
        py_nvosd_text_params.y_offset = 12+y_offset-50
    ...
    for i in range(num_joints + 2):
        ...
        if update[obj_meta.object_id]==False:
...
def tracker_src_pad_buffer_probe(pad, info, user_data):
    ...
    while l_frame:
        ...
        while l_obj:
            ...
            global update
            global squatCount
            if  obj_meta.object_id not in update and obj_meta.object_id not in squatCount:
                update[obj_meta.object_id]=False
                squatCount[obj_meta.object_id]=0
...
def parse_args():
    ...
    parser.add_argument('-w', '--streammux-width', type=int, default=STREAMMUX_WIDTH, help='Streammux width (default: 1920)')
    parser.add_argument('-e', '--streammux-height', type=int, default=STREAMMUX_HEIGHT, help='Streammux height (default: 1080)')
